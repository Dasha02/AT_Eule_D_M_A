...
#include "../../src/plugins/KMPZ/pz_creator/pz_creator.h"
...
string knowledge_field_t::types_to_string() const {
    ...
    for (const auto& type_info : all_types) {
        ...
        if (fuzzy_it != typesFuzzy.end()) {
            ...
            string tempRes = "";
            bool isFirst = true;
            for (const fuzzy& fz : fuzzy_it->second) {
                i++;
                if (isFirst){
                    res += "\"" + fz.fuzzyLex.lvName.toStdString() + "\"\n";
                    isFirst = false;
                }
                auto fuzzyLex = fz.fuzzyLex;
                auto lvData = fuzzyLex.lvData[fuzzyLex.currentlvData];

                string value = "\"" + lvData.name.toStdString() + "\" "
                            + std::to_string(lvData.minValue) + " "
                            + std::to_string(lvData.maxValue) + " "
                            + std::to_string(lvData.points.size()) + " ="
                            + lvData.resultFP.toStdString()+"\n";
                tempRes += value;  // Добавляем сформированную строку в результат
            }
            res += std::to_string(i)+"\n" + tempRes;
...
string knowledge_field_t::types_to_xml() const {
    ...
    for (const auto& it : type_ids) {
        ...
        if (iter2 != typesFuzzy.end()){
            res += "<type id=\"ТИП" + std::to_string(it.second) + "\" meta=\"fuzzy\""+ " type=\""+type_name+"\" "+ "desc=\"string\">\n";
...
string condition_attr_value_const_t::to_rule_xml(const knowledge_field_t& kf) const {
    ...
    res += "<ref id=\"" + obj_desc.second + "\"/>\n";
    res += "</ref>\n";
    res += "<value>" + value + bz_rewrite_in_knowledge(value) + "</value>\n";
    res += "</" + get_text_operation() + ">\n";
    return res;
}
...
void knowledge_field_t::add_type_value(const fuzzy& value) {
    typesFuzzy[value.fuzzyLex.lvName.toStdString()].insert(value);
}
...
//    return res;
//}
string knowledge_field_t::getAllFuzzyValues() const {
        ...
        if (fuzzy_it != typesFuzzy.end()) {
            ...
            for (const fuzzy& fz : fuzzy_it->second) {
                ...
                string value = "\"" + lvData.name.toStdString() + "\" "
                            + std::to_string(lvData.points.size()) + " ="
                            + lvData.resultFP.toStdString()+"\n";
                interval = "ДИАПАЗОН "+std::to_string(lvData.minValue) + " "
                        + std::to_string(lvData.maxValue)+"\n";
                measurment = "ЕДИНИЦЫ ИЗМЕРЕНИЯ "+lvData.measureItem.toStdString()+"\n";
                comment = "КОММЕНТАРИЙ " + fuzzyLex.lvName.toStdString()+"\n\n";
                tempRes += value;
            }
...
set<string> knowledge_field_t::get_type_values(const string& tname) const {
    ...
    if (iter2 != typesFuzzy.end()) {
        for (const fuzzy& fz : iter2->second) {
            auto fuzzyLex = fz.fuzzyLex;
            auto lvData = fuzzyLex.lvData[fuzzyLex.currentlvData];
            ...
            string value = "\"" + lvData.name.toStdString() + "\" "
                        ...
                        + std::to_string(lvData.points.size()) + " ="
                        + lvData.resultFP.toStdString();
            res.insert(value);  // Добавляем сформированную строку в результат
        }
