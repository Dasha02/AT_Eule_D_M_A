...
// Данные лингвистической переменной (LV)
struct LVData {
    QString name; // например высокая
    double minValue;
    double maxValue;
    QString type;
    QString measureItem;
    QVector<Part> points; // Точки ФП
    QString resultFP; // строка с ФП для этого терма
};

// Результат для каждого FuzzyLexeme
struct FuzzyLexemeResult {
    QString term;       // Первое слово из val (например, "ВЫСОКАЯ") текущий терм
    QString lvName;     // Остальные слова из val (например, "ТЕМПЕРАТУРА") текущий
    QString type;       // Тип ФП (например, "type3") текущий
    vector<LVData> lvData;      // Данные LV всех термов
    int currentlvData;
};
class fuzzy{
public:
    FuzzyLexemeResult fuzzyLex;
    string set_symptom;
    string set_answer;
    bool operator<(const fuzzy& other) const {
        if (fuzzyLex.lvData[fuzzyLex.currentlvData].name != other.fuzzyLex.lvData[other.fuzzyLex.currentlvData].name )
            return fuzzyLex.lvData[fuzzyLex.currentlvData].name < other.fuzzyLex.lvData[other.fuzzyLex.currentlvData].name;
        return false;
    }
};
...
private:
    ...
    map<string, set<fuzzy>> typesFuzzy;
...
public:
    ...
    void add_type(const fuzzy& type_name);
    void add_type_value(const fuzzy &value);
