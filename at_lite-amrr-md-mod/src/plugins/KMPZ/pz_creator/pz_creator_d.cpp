...
//        std::cout << "\n единицы измерения = " << emptyItem.units_of_measurement << "категория измерения" << emptyItem.category;
    ...
    for (int i = 0; i < fuzzyInaccurateList.count(); ++i) {
        ...
        item.units_of_measurement = units.toStdString();
...
//        std::cout << "\n=== НАЙДЕН СИМПТОМ: [" << symptom.toStdString() << "]\n";
//        std::cout << ">>> cleanSymptom: [" << cleanSymptom.toStdString() << "]\n";
...
//            std::cout << "  >> Сравниваем с: [" << itemSymptom.toStdString() << "]\n";
...
//                std::cout << "     >> СОВПАДЕНИЕ!\n";
                ...
                {
//                    std::cout << "     >> ПРОХОДИТ УСЛОВИЕ (lemma / error_rate): "
//                              << item.indefinite_lemma << " / " << item.error_rate << "\n";

                    result += fullMatch + "\n\n";
                    match_found = true;
                    break;
                } else {
//                    std::cout << "     >> НЕ проходит условие: lemma пустая и error_rate == 0\n";
                }
            }
        }
...
//            std::cout << "  >> НЕТ СОВПАДЕНИЙ\n";
        ...
        offset = match.capturedEnd(0);  // важно: всегда двигаем offset
    }
