...
std::vector<fuzzy> PZCreator::getAllFuzzyValues() const{
...
void SI_in_txt_1(std::string a, vector<fuzzy> fuz) {
    ...
    string result;
    for (auto fvalue : fuz) {
        auto current = fvalue.fuzzyLex.lvData[fvalue.fuzzyLex.currentlvData];
        result +="НЕЧЕТКИЙ АТРИБУТ \n"
                 "ЗНАЧЕНИЯ\n"+ fvalue.fuzzyLex.term.toStdString() +" "+ current.resultFP.toStdString() +"\n" +
                 "ДИАПАЗОН " + std::to_string(current.minValue) +" "+std::to_string(current.maxValue)+"\n"+
                 "ЕДИНИЦЫ ИЗМЕРЕНИЯ "+current.measureItem.toStdString()+"\n"+
                 "КОММЕНТАРИЙ "+fvalue.set_answer+"\n"
        ;
    }
    QString resultFuzzy = QString::fromStdString(result);
    // == Блок Алексея (конец)
